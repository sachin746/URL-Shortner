...
// // // @Description Redirects the user to Github OAuth2 authentication page
// // // @Tags auth
...
func HandleGithubAuth(c *gin.Context) {
	...
	url, err := business.GetGoogleGithubAuthURL(c, constants.Github)
	if err != nil {
	...
	// Redirect the user to the Github OAuth2 authentication page
	c.Redirect(http.StatusTemporaryRedirect, url)
	log.Sugar.Info("Redirecting to Github OAuth2 authentication page")
}

// HandleGithubAuthCallback godoc
// // @Summary Handle Github OAuth2 callback
// // // @Description Handles the callback from Github OAuth2 after user authentication
// // // @Tags auth
...
func HandleGithubAuthCallback(c *gin.Context) {
	// Handle the callback from Github OAuth2
	user, token, err := business.HandleGithubAuthCallback(c)
	if err != nil {
		log.Sugar.Errorf("Failed to handle github OAuth callback: %v", err)
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to authenticate with Github"})
		return
	}

	if user == nil {
		log.Sugar.Error("User not found in github authentication callback")
		c.JSON(http.StatusUnauthorized, utils.ErrorResponse(errors.New("user not found")))
		return
	}

	log.Sugar.Infof("User %s authenticated successfully with github", user.Username)
